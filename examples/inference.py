    ...
    def __init__(
        ...
        if is_chunked:
            ...
            for i in range(num_chunks):
                ...
                if verbose:
                    print(f"  Loading chunk {i}: {chunk_path}")
                model = ct.models.MLModel(str(chunk_path))
                self.models.append(model)
                self.states.append(model.make_state())
        else:
            if verbose:
                print(f"Loading model from {model_dir}...")
            self.models = [ct.models.MLModel(str(model_dir))]
            self.states = [self.models[0].make_state()]

        # Get sequence length from model input spec
        input_spec = self.models[0].get_spec()
        # Find the inputs_embeds or hidden_states input
        for input_desc in input_spec.description.input:
            if input_desc.name in ["inputs_embeds", "hidden_states"]:
                # Shape is (batch, hidden_dim, 1, seq_len)
                self.seq_len = input_desc.type.multiArrayType.shape[-1]
                break
        else:
        ...
        if max_context_length is None:
            ...
            # State shape is typically (batch, num_heads, max_context, head_dim)
            # or (batch, max_context, num_heads, head_dim)
            detected_context_length = None
            for state_desc in input_spec.description.state:
                state_shape = list(state_desc.type.multiArrayType.shape)
                # KV cache typically has shape with max_context as one dimension
                # Common patterns: [1, num_heads, max_ctx, head_dim] or [1, max_ctx, num_heads, head_dim]
                # The max_context is usually the largest dimension after batch
                if len(state_shape) >= 3:
                    # Find the dimension that's likely max_context (not batch=1, not small head dims)
        ...
        # Load LM head
        if verbose:
            print(f"Loading LM head from {lm_head_path}...")
        self.lm_head = ct.models.MLModel(str(lm_head_path))
        if verbose:
